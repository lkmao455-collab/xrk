...
- **实时屏幕标注（v1.6.0）**：控制端在共享画面上自由笔标注，实时同步并在被控端物理屏以透明 overlay 显示（穿透点击、不影响对方操作），支持颜色/宽度与一键清空。
- **Web 触控与文件管理（v1.7.0）**：Web 客户端支持触控手势（单击/拖拽/长按右键/双指捏合与平移滚动）操作远程桌面；「快照」按钮将当前画面与标注导出为 PNG；远程文件管理器新增重命名/删除/新建文件夹（写操作需被控端授权并写审计日志），并补强远程重启/关机（POWER_COMMAND）的授权门禁。
