...
- [ ] **Task 31** 移动端客户端 —— 记录为长期路线图（本次不实现，见下方记录）
...
##### Android平台
- **屏幕采集**：需使用`MediaProjection` API（Android 5.0+）
- **输入控制**：需使用`AccessibilityService`或`Instrumentation`（需用户授权）
- **编解码**：需集成H.264/H.265硬件编码（MediaCodec）
- **网络**：需实现TCP/UDP通信，处理NAT穿透

##### iOS平台
- **屏幕采集**：需使用`ReplayKit`框架（iOS 11+）
- **输入控制**：需使用`Private API`（App Store审核风险）或`Accessibility`
- **编解码**：需使用`VideoToolbox`硬件编码
- **网络**：需实现TCP/UDP通信，处理NAT穿透

#### 3. 架构差异
- **当前架构**：Windows桌面应用，Qt/C++实现
- **移动端架构**：原生应用（Android Kotlin/Java，iOS Swift/ObjC）
- **协议兼容**：需保持与现有Windows版本的协议兼容
- **代码复用**：核心协议和加密库可复用，UI需重写

#### 4. 实现复杂度
- **Android版**：高（2-3个月，含测试）
- **iOS版**：高（2-3个月，含测试）
- **跨平台框架**：可考虑Flutter/React Native，但性能可能受影响

#### 5. 资源需求
- **开发团队**：需Android/iOS开发工程师
- **测试设备**：需多种Android/iOS设备
- **应用商店**：需注册Google Play和Apple Developer账号
- **审核周期**：iOS App Store审核严格，可能被拒

### 长期路线图建议

#### 阶段1：Android版（优先）
- **理由**：Android用户基数大，开发门槛相对较低
- **时间**：预计6-9个月后启动
- **功能范围**：基础远程控制（屏幕查看、输入控制、文件传输）
...
#### 阶段3：功能对齐
- **目标**：移动端功能与Windows版完全对齐
- **时间**：预计18-24个月后

### 当前不实现的理由
1. **架构差异大**：移动端需要原生开发，无法直接复用Qt代码
2. **开发成本高**：需要专业的移动端开发团队
3. **审核风险**：iOS App Store审核可能拒绝远程控制类应用
4. **核心功能优先**：当前应聚焦Windows版功能完善和稳定性
5. **用户基数**：当前用户主要在Windows平台，移动端需求不迫切

### 替代方案
- **推荐**：用户使用现有的移动端远程控制应用（如TeamViewer、AnyDesk）配合XRK Windows版
- **未来**：如用户需求强烈，可评估使用Flutter等跨平台框架开发
