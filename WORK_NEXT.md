...
- [x] **Task 24** 剪贴板双向同步 + 自动粘贴 — 已提交 `052fd6d`（host 端新增 ClipboardManager 监控本机剪贴板并广播给所有认证客户端；接收端 applyRemoteClipboard 带循环防护；控制器侧也已用其避免回播。自动粘贴=接收即写入本机剪贴板，用户可直接 Ctrl+V，未做强制自动按键粘贴因安全风险）
